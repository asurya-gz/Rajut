...
<body class="font-sans antialiased bg-pink-100">
    @php
        $navClasses = function ($active) {
            return $active
                ? 'flex items-center px-4 py-3 text-white bg-green-700 rounded-lg'
                : 'flex items-center px-4 py-3 text-white hover:bg-green-700 rounded-lg transition-colors duration-150';
        };
    @endphp
    <div class="min-h-screen flex">
        <aside class="w-64 bg-green-600 text-white fixed inset-y-0 left-0 overflow-y-auto">
            <div class="p-6">
            ...
            <nav class="mt-8 h-full flex flex-col pb-6">
                <div class="px-4 space-y-2">
                    <a href="{{ route('admin.dashboard') }}" class="{{ $navClasses(request()->routeIs('admin.dashboard')) }}">
                    ...
                    <a href="{{ route('admin.products.index') }}" class="{{ $navClasses(request()->routeIs('admin.products.*')) }}">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('admin.orders.index') }}" class="{{ $navClasses(request()->routeIs('admin.orders.*')) }}">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="border-t border-green-500 my-6 mx-4"></div>

                <div class="px-4 space-y-2">
                    <a href="{{ route('admin.users.index') }}" class="{{ $navClasses(request()->routeIs('admin.users.*')) }}">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <header class="bg-white shadow-sm border-b border-gray-200 flex-shrink-0">
                <div class="flex items-center justify-between px-6 py-4">
                    <div class="flex-1 max-w-lg">
                        <div class="relative">
                            <input type="text" placeholder="Cari..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" @keydown.escape.window="open = false" type="button"
                                    class="relative inline-flex h-10 w-10 items-center justify-center rounded-full border border-green-100 bg-white text-green-600 shadow-sm transition hover:border-green-300 hover:text-green-700">
                            ...
                            <div x-cloak x-show="open" x-transition.origin.top.right
                                 ...
                                 class="absolute right-0 z-50 mt-3 w-80 origin-top-right rounded-2xl border border-green-100 bg-white shadow-xl shadow-emerald-100/60">
                                <div class="flex items-center justify-between border-b border-green-50 px-4 py-3">
                                    ...
                                    <button @click="open = false" class="text-xs font-medium text-green-600 hover:text-green-700">Tutup</button>
                                </div>

                                <div class="max-h-80 overflow-y-auto">
                                    @forelse($recentActivities as $activity)
                                        <div class="flex items-start gap-3 px-4 py-3 text-sm hover:bg-green-50/70">
                                            <span class="mt-1 inline-flex h-8 w-8 items-center justify-center rounded-full bg-green-100 text-green-600">
                                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <p class="mt-1 text-xs font-medium text-green-600">Total: Rp {{ number_format($activity->total, 0, ',', '.') }}</p>
                                            </div>
                                        </div>
                                    @empty
                                ...
                                <div class="border-t border-green-50 px-4 py-2 text-center text-[11px] uppercase tracking-wide text-green-300">
